...
I will use HAL2 libraries during coding phase and during debug session I will demonstrate usage of FreeRTOS debug add-on.

## Example description
Application will run FreeRTOS with two active tasks which I will create:
...
I will start from PA5 pin which is controlling Green LED.
...
Now I have my basic IOs properly setup for the application.

## Clock configuration
I will keep a default clock configuration.

I will work on internal oscillator to reach 144MHz frequency for the core and most of the peripherals I will use

## USART2 configuration
On our board USART2 via pins PA2, PA3 is connected to on-board STLink and can be visible on PC as Virtual COM.
I will use this opportunity to send some data from one of tasks.
I need to activate USART2. I will use most of the default settings, 
...
I can notice there is an issue reported. It is related to selection of SysTick for HAL library and OS tick which is not recommended.
...
To do this I need to swich to Peripherals section once again, go to timers section and select one of free timers. Less featured one is the good choice here (TIM6 or TIM7).
...
I will not change any default settings within its configuration, but it is recommended to do it in the final product to save FLASH/RAM resources by deactivation of OS elements which will not be used in our application.

In my case will just create two tasks and one binary semaphore.
...
Now I am ready to generate our project and do some codig.
...
I need to specify the location where the main MakeLists.txt is located (usually it is the folder with name of our project with _cmake suffix).
...
I need first to include header file with FreeRTOS functions definitions by:
...
vTaskStartScheduler();
```
I need to call FreeRTOS configuration function which will create its components (2 tasks and 1 binary semaphore in our case).
Then I need to start FreeRTOS. Important message is that we should never land with code execution below this line as FreeRTOS is overtaking the code execution between its components (tasks).

Now I need to add some code within my tasks.
LED_blink task should toggle Green LED (PA5) each 200ms and could be stopped by the binary semaphore.
Thus I can organize this application code as below:
...
Thus I can use the following application code:
...
To release (give) the semaphore I will use an interrupt callback assigned to PC13 (blue button).
I can use below code snippet for that:
...
```
Once the coding part is done I can build the application and start the debug session.

## Build the application
To build the application I can save all active files by:

Then I can click on the Build icon on the bottom bar

![Build](_htmresc/build_button.png)

If there is no compilation error I should receive the information about application code and RAM usage.
I can display the details by clicking "piles" button:

![Build](_htmresc/Code_size.gif)

Now I am ready to start the debug
...
I need to activate our RTOS debug plugin before we start a debug. I will update our debug script (called launch.json).
...
once done I can press "play" button.

Once debug session will be active I can see:
- on top small bar with debug control:

![debug_bar](_htmresc/debug_bar.png)

- on the bottom window an additional tab called STM32CUBE_RTOS. It will display data about FreeRTOS components once I pause the code execution (by default there is no live monitor).

To test it.
Start an application, after the while pause it and see an effect on the board and in STM32CUBE_RTOS window.

That's it. I can stop debug session, by pressing STOP icon
